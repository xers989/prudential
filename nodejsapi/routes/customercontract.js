...
router.route('/:customer/process').get( async(req, res, next) => {
    try{
        let query = "";
        let _customer = parseInt(req.params.customer);
        let _destory_id = req.query.destory_id;
        ...
        //let _projection = {_id:0};
        //query = {u_customer_no:_customer};
        //query = {n_customer_no:_customer, request_customer_destroy_id: _destory_id};


        await client.connect();
        const database = client.db(databasename);
        const customerCollection = database.collection("customer_destory");

        query = {u_customer_no:_customer, request_customer_destroy_id: _destory_id};
...
.post(async (req, res, next) => {
    ...
    try{
        ...
        await customerCollection.updateOne(
            query,
            { $set:  {request_customer_destroy_process: exampleDocument }}
          );
